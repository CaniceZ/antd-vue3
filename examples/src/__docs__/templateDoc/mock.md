...
```bash
# vite 本地跨域代理
VITE_SERVICE_URL = http://gateway-test1.yigongpin.net
# 接口地址
...
- VITE_SERVICE_URL 不能换行

:::

### 跨域处理

如果你在 `src/api/` 下面的接口为下方代码，且 **.env.development** 文件配置如下注释，则在控制台看到的地址为 `http://localhost:3100/api/login`。

由于 `/api` 匹配到了设置的 `VITE_SERVICE_URL`，所以上方实际是请求 **http://localhost:3000/login**，这样同时也解决了跨域问题。（**3100**为项目端口号，**http://localhost:3000**为 PROXY 代理的目标地址）
...
如果没有跨域问题，可以直接忽略 **VITE_SERVICE_URL** 配置，直接将接口地址设置在 **VITE_GLOB_API_URL**
...
如果有跨域问题，将 **VITE_GLOB_API_URL** 设置为跟 **VITE_SERVICE_URL** 一致的值即可。

下方的接口地址设置为 `/api`，当请求发出的时候会经过 Vite 的 proxy 代理，匹配到了我们设置的 **VITE_SERVICE_URL** 规则，将 `/api` 转化为 `http://gateway-test1.yigongpin.net` 进行请求
