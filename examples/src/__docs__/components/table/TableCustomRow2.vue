  ...
  list.forEach((row: any) => {
    if (row.skuList) {
      ...
      if (row.skuList.length > keepNum) {
        ...
        onExpand(row.skuList[row.skuList.length - 1])
      }
    }
  })
  return {
...
function onExpand(record) {
  ...
  if (record.isExpand) {
    dataMap[parent.id] = parent.skuList.splice(
      keepNum,
      parent.skuList.length - (keepNum + 1)
    )
  } else {
    parent.skuList.splice(parent.skuList.length - 1, 0, ...dataMap[parent.id])
    delete dataMap[parent.id]
  }
  ...
  if (record.skuCode) {
    if (column.dataIndex === 'price') {
      ...
      if (record === record.parent?.skuList?.[0]) {
        const length = record.parent.skuList.length
        const last = record.parent.skuList[length - 1]
        return {
          colSpan: 1,
          rowSpan: last.isBtn ? length - 1 : length,
...
function rowClassName(record: any) {
  if (record.skuCode || record.isBtn) {
    return 'sku-row'
  } else {
...
<template>
  ...
  >
    <template #body-cell-skuCode="{ record }">
      ...
      <template v-else-if="record.isBtn">
        <y-btn block link small @click="onExpand(record)">
          <template v-if="record.isExpand">点击收起 <UpOutlined /></template>
          <template v-else>点击展开 <DownOutlined /></template>
        </y-btn>
      </template>
      <template v-else>
        <y-space>
