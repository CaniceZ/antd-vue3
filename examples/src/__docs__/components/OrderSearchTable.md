# y-order-search-table 订单搜索表格

> 关于取名 `y-order-search-table`：想了好多关于该场景样式的组件名称，均不好记忆，最终取名为 `order-table`，一是因为属于 `y-search-table` 组件的衍生，二是因为这个最先应用在订单平台，欢迎有相似场景需求的都可以使用该组件，不一定是订单相关的需求

继承 `y-search-table` 的所有属性、插槽、实例方法，目前适用在订单中台的表格查询样式场景
...
// @ts-ignore
import { useSelection, useFormPage } from '@ygp/ygp-design-vue/hooks'
...
const { formData, formTableProps } = useFormPage(getPage, { pageSize: 2 })

const items = computed<FormItem[]>(() => [{ label: 'SKU号', name: 'skuCode' }])
